...
from src.models import Evidence, EvidenceType, Language, Confidence, SourceReference, Transcript, TranscriptSegment, TranscriptLanguage

class TranscriptionEngine:
    def transcribe(self, audio_path, language="unknown"):
...
def transcript_to_evidence(transcript):
    return transcript.to_transcript_evidence()

def create_transcript_from_segments(material_id, segments_data, language="unknown"):
    lang = TranscriptLanguage.from_string(language)
    segments = []
