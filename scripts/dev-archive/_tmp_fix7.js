...
console.log("removing:",removeSet.size);
if(removeSet.size!==34){process.exit(1);}

const kept=lines.filter((_,i)=>!removeSet.has(i));

// ---- find TR block closes in kept[] (bounded loop) ----
function findTRClose(arr,lang,from){
...
const trK=kept.findIndex(l=>l.indexOf("const TRANSLATIONS")===0);
const esEnd=findTRClose(kept,"es",trK);
console.log("esEnd:",esEnd);
if(esEnd<0){process.exit(1);}
// ca block: search from esEnd (after es close)
const caEnd=findTRClose(kept,"ca",esEnd);
console.log("caEnd:",caEnd);
if(caEnd<0){process.exit(1);}
