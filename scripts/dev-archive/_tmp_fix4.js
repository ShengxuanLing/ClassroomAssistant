...
// exact-match keys (line trims to a single 'KEY': line) — 13 of them
const exactKeys=keysToRemove.slice(0,13);
const removeSet=new Set();
for(const k of exactKeys){
  let n=0;
  lines.forEach((l,i)=>{ if(i>=i18nS&&i<=i18nE){ const t=l.trim(); if(t.startsWith("\u0027"+k+"\u0027: ")){n++; removeSet.add(i);} } });
  if(n!==2) console.log("!! exact key "+JSON.stringify(k)+" found "+n);
}
// long keys 14-16: substring match on key + colon
for(const k of keysToRemove.slice(13)){
  let n=0;
  lines.forEach((l,i)=>{ if(i>=i18nS&&i<=i18nE){ if(l.indexOf("\u0027"+k+"\u0027: ")===4){n++; removeSet.add(i);} } });
  if(n!==2) console.log("!! long key "+JSON.stringify(k)+" found "+n);
}
// orphan 错误 in TR region
lines.forEach((l,i)=>{ if(i>=trS&&i<=trE&&l.trim().startsWith("\u0027\u9519\u8bef\u0027: ")){removeSet.add(i);} });
console.log("removing total:",removeSet.size,"(expect 34)");
...
const out=[...kept];
const caL=caNew.map(([k,v])=>"    \u0027"+k+"\u0027: \u0027"+v.replace(/\u0027/g,"\\u0027")+"\u0027,");
const esL=esNew.map(([k,v])=>"    \u0027"+k+"\u0027: \u0027"+v.replace(/\u0027/g,"\\u0027")+"\u0027,");
