...
- **2026-09-22（课程页课堂列表重设计：日期分组 + 卡片；清除"图中未显示"）**：用户截图报课程详情页课堂区是 8 列密集大表格（#/标题/日期/状态/材料/知识点/待审核/操作），且标题里渲染出技术占位符"图中未显示"。**UI**：`views/courses.js::pageCourse` 的课堂区从 `<table>` 改为「一行 summary（`28 节课堂 · 2026-09-09 → 2026-12-09`）→ 按日期分组（组头 `09/09 · 周三`）→ 轻卡片」。卡片 = 时间 + 类型徽章 / 课堂标题 / 教室 + 状态 pill，另有一行**非 0 才画**的计数 meta（`材料 2 · 知识点 8 · 待审核 3`）。新解析器 `parseSessionTitle` 把种子格式 `时间 类型 | 教师 | 教室` 拆回结构化字段：标题优先取教师段、缺教师段退回类型（GEO 迁移后正是任务要求的 `15:00–17:00 / Teoria / Aula Q1/1007`）；类型徽章只在标题≠类型时画（防重复）；空标题兜底新 key `未命名课堂`（×3 语），summary 用新 key ` 节课堂`（×3 语）。整卡可点用 stretched-link（`.session-card-main::after` 铺满卡片），「整堂处理」按钮是**兄弟节点** + `z-index:1`，不嵌 `<a>`；href / `data-action="process-session"` / confirmDestructive 接线与旧表格逐字一致。CSS 新增 `.session-summary/.session-day(.+-head)/.session-list/.session-card*`，`<=560px` 卡片换行不横向滚动。**Data**：占位符源头 = `temp/create_schedule_sessions.py` 把 GEO 两行教师栏写成"图中未显示"（转录课表图时教师不可见）并编进 title。脚本已改（教师空 → 整段省略），但 `create_session` 幂等返回既有记录、**不覆盖 title**，API 重放修不了旧行 —— 停服后跑 `temp/fix_session_titles.py --apply` 迁移库内 28 条（title 与 payload 两列同改；142 条总数不变、残留 0；重启后 API 实测 0 条）。渲染侧再兜未迁移旧库：解析层丢弃空段/占位段，**不是** CSS 隐藏。**两个实测坑**：① `Aulas?` 手误成 `Aules?`（匹配 "Aule"）→ 两段式教室识别静默失败、房间被当标题；是 audit 的「徽章恰 1 个」断言抓住的（当时 3 张卡都画了徽章）—— 房间串恰好也出现在渲染里，`includes('Aula …')` 类断言会假通过，**语义断言才有分辨力**。② audit 标记写 `class="session-summary"` 会匹配不到（属性里还有 small/muted 前缀），要用 `session-summary">`。**守卫**：`ui_audit.js` +27 条 → **469 全过**；`ui_render_check.js` **360 全过**。真实浏览器（无头 Edge `--dump-dom`；必须配独立 `--user-data-dir`，否则单实例转发导致输出 0 字节）实测 GEO 真数据页：14 个日期组 / 28 张卡 / 组头 `09/09 · 周三…10/14 · 周三` / summary / 占位符 0 / 按钮与链接各 28。Python 受影响面六个文件（exercise/learning i18n、student_ui、web_ui、web_ui_invariants、multi_course）**513 passed / 0 failed**（5m29s）。
